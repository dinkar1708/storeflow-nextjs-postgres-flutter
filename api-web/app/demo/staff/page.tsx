...
const queue = [
  ...
  { id: 'ORD-9418', customer: 'Marco Diaz', items: 5, total: 312.4, age: '5h', status: 'PACKED' },
  {
    id: 'ORD-9415',
  ...
  { id: 'ORD-9410', customer: 'Liang Wei', items: 4, total: 220.0, age: '1d', status: 'SHIPPED' },
  {
    id: 'ORD-9402',
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
        ...
        <section className="bg-white rounded-xl shadow p-6">
          ...
          <p className="text-gray-600 mt-1">
            Move orders through the pipeline. Update stock as you pick.
          </p>
        </section>
        ...
        <section className="bg-white rounded-xl shadow">
          <div className="p-6 border-b flex justify-between items-center">
            ...
            <div className="flex gap-2 text-sm">
              ...
              <button className="px-3 py-1.5 border rounded-md bg-yellow-50 text-yellow-800 border-yellow-200">
                Processing
              </button>
              <button className="px-3 py-1.5 border rounded-md bg-purple-50 text-purple-800 border-purple-200">
                Packed
              </button>
            </div>
          </div>
        ...
        <section className="bg-white rounded-xl shadow p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {lowStock.map((s) => (
              <div
                key={s.sku}
                className={`rounded-xl p-4 border ${s.stock === 0 ? 'border-red-200 bg-red-50' : 'border-yellow-200 bg-yellow-50'}`}
              >
                <div className="flex justify-between items-start">
                  <div>
                  ...
                  <span
                    className={`text-2xl font-bold ${s.stock === 0 ? 'text-red-600' : 'text-yellow-700'}`}
                  >
                    {s.stock}
                  </span>
                </div>
                <div className="mt-3 text-xs text-gray-600">Threshold: {s.threshold}</div>
                <button className="mt-3 w-full px-3 py-1.5 bg-blue-600 text-white text-sm rounded-md">
                  Update stock
                </button>
              </div>
            ))}
